...
So với underfit, mô hình good fit giảm Test MAE từ 2.669 xuống 2.284 bước, tương đương cải thiện khoảng 14.4\%. So với overfit, good fit cũng tốt hơn khoảng 25.5\% theo MAE. Điều này củng cố nhận xét rằng mô hình vừa phải, có regularization và dừng sớm phù hợp hơn với dataset của project.
\end{resultbox}
...
Mặc dù A* + ANN có thời gian chạy hơi nhỉnh hơn một chút so với tính toán biểu thức Manhattan thuần túy do quá trình xử lý của mô hình, nhưng nhờ áp dụng TensorFlow Lite, tốc độ inference trên CPU vẫn diễn ra rất nhanh (khoảng 0.48ms trong demo). Điều này cho thấy tiềm năng áp dụng AI vào thuật toán tìm đường khi chi phí suy luận được kiểm soát.

Ngoài ảnh demo tĩnh, project có thêm giao diện tương tác tại \texttt{src/ui.py}. Giao diện này cho phép random map mới, chạy từng thuật toán hoặc chạy tất cả, chọn thuật toán để xem lại node đã duyệt, đường đi cuối cùng, số node và thời gian chạy.
...
\section{Mức độ sử dụng AI hỗ trợ và sản phẩm nộp}

\subsection{Mức độ sử dụng AI hỗ trợ}

Trong quá trình thực hiện, AI được dùng như một công cụ hỗ trợ kỹ thuật, không thay thế phần phân tích và phản biện của nhóm. Cụ thể, AI hỗ trợ gợi ý cấu trúc project, tạo khung code ban đầu, giải thích lỗi khi debug và đề xuất cách tổ chức nội dung báo cáo.

Các quyết định chính của đề tài vẫn cần nhóm tự nắm và bảo vệ, gồm: thiết kế input/output của ANN, cách sinh nhãn bằng BFS, lý do chia train/validation/test, cách đọc đồ thị loss/MAE, nguyên nhân underfitting/overfitting, và hạn chế của ANN khi dùng làm heuristic cho A*.

\begin{keybox}{Cam kết khi bảo vệ}
Nhóm đã chạy lại code, dùng số liệu thực tế trong thư mục \texttt{outputs/}, và không trình bày ANN như một phương pháp luôn tốt hơn heuristic truyền thống. Kết quả được hiểu là minh họa thực nghiệm cho việc kết hợp ANN với thuật toán A*.
\end{keybox}

\subsection{Sản phẩm nộp}

\begin{table}[H]
\centering
\caption{Danh sách sản phẩm nộp kèm project}
\begin{tabularx}{0.95\textwidth}{>{\bfseries\raggedright\arraybackslash}p{0.28\textwidth}Y}
\toprule
Hạng mục & File hoặc thư mục tương ứng \\
\midrule
Báo cáo PDF & \texttt{reports/ann\_astar\_report.pdf} \\
Báo cáo nguồn & \texttt{reports/ann\_astar\_report.tex} và \texttt{reports/report\_draft.md} \\
Slide thuyết trình & \texttt{slides/slide\_content.md} và \texttt{slides/slide\_outline.md} \\
...
Kết quả thực nghiệm & Thư mục \texttt{outputs/} gồm biểu đồ loss, metrics và ảnh demo \\
